...
from common.exception import InvalidValueException, AccessDeniedException
...
class HttpUtil(object):
    ...
    def get(url, timeout=None, auth=None, headers=None):
        ...
        logging.debug("API Call: GET %s, Time: %s" % (url, DatetimeUtil.diff(start_time, datetime.datetime.now())))
        r.raise_for_status()
        return r
    ...
    def post(url, data=None, timeout=None, auth=None, headers=None):
        ...
        logging.debug("API Call: POST %s, Time: %s" % (url, DatetimeUtil.diff(start_time, datetime.datetime.now())))
        r.raise_for_status()
        return r
